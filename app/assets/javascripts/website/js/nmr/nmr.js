...
(function( global, factory ) {
	...
	var factory = function( Graph, Attribution, JcampConverter, Molecule ) {
		...
		function fetchUrls( nmr, urls, options ) {
			...
			var fetching = [];
			for( var i in urls ) {
			    fetching.push($.get(urls[i]).then(function (data) {
			        return JcampConverter.convert(data, {
			            keepSpectra: true
			        })
			    },
                function (err)
		...
		NMR.prototype.loaded = function( series, options, name ) {
			...
			switch( this.getMode() ) {
				...
				case '1d':
					...
					this.setSerieX( name, series.x.spectra[ 0 ].data[ 0 ], { label: " " } );
...
console.log( from, to );
    ...
    if( typeof define === "function" && define.amd ) {
        ...
        define( [ 'graph', 'assignation', 'jcampconverter', 'sd' ], function( Graph, Assignation, JcampConverter, SD ) {
