...
function initProv(prov, city, defaultProv, defaultCity) {
    ...
    var provHtml = '';
    provHtml += '<option value="">省份</option>';
    ...
    if(defaultProv != ''){
        provEl.parent().find("div.text").html(defaultProv);
    }
...
function initCities(provEl, cityEl, defaultCity) {
    ...
    if(provEl.val() != '' && parseInt(provEl.val()) !="" ) {
        for(var i = 0; i <_cityInfo.length; i++) {
        ...
        var cityHtml = '';
        cityHtml += '<option value="">城市</option>';
        ...
        if(defaultCity != ''){
            cityEl.parent().find("div.text").html(defaultCity);
        }
    } else {
        cityEl.html('<option value="">城市</option>');
    }
};
